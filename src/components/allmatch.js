...
export default class Allmatch extends React.Component {
  constructor() {
    super();
  ...
  componentDidMount() {
    ...
    var array = [];
    // db reference
    firebase.database().ref("/").child("regMatch").on('child_added', snap => {
      obj = snap.val();
      obj.key = snap.key
    ...
    //   firebase.database().ref("/").child('regMatch/' + id +"/").child("id/").on("child_added",(snap)=>{
    //     var  checkobj=snap.val();
    //            obj.length();
    ...
    if (this.state.count < team) {
      ...
      firebase.database().ref('users/' + user + '/').once('value')
        .then((data) => {
          ...
          firebase.database().ref("/").child('regMatch/' + id + "/").child("id/").push(regobj)
            ...
            return (
              <Content key={index} id={obj.key} >
                <Card>
                  ...
                  <CardItem>
                    <Body>
                      <Text style={{ fontSize: 18 }}>Entry :<Text> {value.entery}PKR</Text> </Text>
                      <Text style={{ fontSize: 18 }}>Match Timing :<Text> {value.time}</Text></Text>
                      <Text style={{ fontSize: 18 }}>Address :<Text> {value.area}</Text></Text>
                      <Text style={{ fontSize: 18 }}>Total Team :<Text> {value.teams}</Text></Text>
                      <Text style={{ fontSize: 18 }}>Date : <Text>{value.date}</Text></Text>
                      <Text style={{ fontSize: 18 }}>Rules Details : <Text>{value.details}</Text></Text>
                    </Body>
                  </CardItem>
                  <CardItem footer>
